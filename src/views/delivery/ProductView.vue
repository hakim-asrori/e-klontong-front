...
const productFilters = ref({
  page: 1,
  per_page: 10,
  delivery_service: $props.delivery == 'udara' ? 2 : 1
})
...
const getProduct = async () => {
  ...
  try {
    const response = await $store.dispatch("postData", ["products", productFilters.value])
...
<template>
  <TitleBar :title="delivery ?? 'Category'" :show-back="true" />
