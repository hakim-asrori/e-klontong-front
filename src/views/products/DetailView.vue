...
import { CButton, CCard, CCardBody, CCardImage, CModal, CModalBody, CModalFooter, CModalHeader, CSpinner } from '@coreui/vue';
...
const addToCart = async (product) => {
  ...
  try {
    const response = await $store.dispatch("postData", ["cart/store", {
...
<template>
  ...
  <div class="d-flex align-items-center justify-content-center mt-5" v-if="isLoading">
    <CSpinner class="mb-3" />
  </div>
  <div v-else>
    <a href="#" @click="() => { showImage = true }">
      ...
      <CCard class="shadow border-0">
        <CCardBody class="p-2">
          <div class="d-flex align-items-center gap-2">
            ...
            <span v-if="product.categories">{{ product.categories.join(", ") }}</span>
          </div>
          <hr class="my-2">
          <div class="d-flex align-items-center gap-2">
            <i class="fa-solid fa-sm fa-star text-warning"></i>
            <span>{{ (product.totalSales / 5) < 1 ? 'Belum ada Penjualan' : (product.totalSales / 5) }}</span>
          </div>
          <hr class="my-2">
          <div class="d-flex align-items-center gap-2">
    ...
    <CCard class="fixed-bottom rounded-0 border-top border-0">
      <CCardBody>
        <button class="w-100 btn btn-primary d-flex align-items-center justify-content-between"
  ...
  <CModal :visible="showImage" @close="() => { showImage = false }" aria-labelledby="LiveDemoExampleLabel">
    <CModalBody>
      <img :src="product.image" width="100%" height="300">
    </CModalBody>
    <CModalFooter>
      <CButton color="secondary" @click="() => { showImage = false }">
        Close
      </CButton>
    </CModalFooter>
  </CModal>
