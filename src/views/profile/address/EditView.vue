...
const getAddress = async () => {
  try {
    ...
    if (response.code == $store.state.STATUS_CODE.DATA_OK) {
      address.value.detail = response.data.detail
      address.value.name = response.data.name
      address.value.phone = response.data.phone
      address.value.is_default = response.data.isDefault
      address.value.province_id = response.data.provinceId
      address.value.district_id = response.data.districtId
      address.value.village_id = response.data.villageId
      address.value.regency_id = response.data.regencyId
      address.value.provinceName = response.data.provinceName
      address.value.districtName = response.data.districtName
      address.value.villageName = response.data.villageName
      address.value.regencyName = response.data.regencyName
    }
  } catch (error) {
  }
}
      ...
      <CCardBody>
          ...
          <vSelect label="name" value="id" id="province" placeholder="Choose Province" :options="provinces"
            ...
            <template #search="{ attributes, events }">
              <input class="vs__search" :required="!address.province_id" v-bind="attributes" v-on="events"
                ...
                :value="provinceSelected ? provinceSelected.name : address.provinceName"
          ...
          <vSelect label="name" value="id" id="regency" :disabled="address.province_id < 1 || isLoading"
            ...
            <template #search="{ attributes, events }">
              <input class="vs__search" :required="!address.regency_id" v-bind="attributes" v-on="events"
                :disabled="isLoading" :class="{ 'is-invalid': validation.regency_id }" :value="address.regencyName"
          ...
          <vSelect label="name" value="id" id="district" :disabled="address.regency_id < 1 || isLoading"
            ...
            <template #search="{ attributes, events }">
              <input class="vs__search" :required="!address.district_id" v-bind="attributes" v-on="events"
                :disabled="isLoading" :class="{ 'is-invalid': validation.district_id }" :value="address.districtName"
          ...
          <vSelect label="name" value="id" id="village" :disabled="address.district_id < 1 || isLoading"
            ...
            <template #search="{ attributes, events }">
              <input class="vs__search" :required="!address.village_id" v-bind="attributes" v-on="events"
                :disabled="isLoading" :class="{ 'is-invalid': validation.village_id }" :value="address.villageName"
        ...
        <div class="mb-3">
          <CFormCheck id="flexCheckDefault" label="Set Default" v-model="address.is_default" :disabled="isLoading"
            :class="{ 'is-invalid': validation.is_default }" :checked="address.is_default" />
